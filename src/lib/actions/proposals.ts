...
import {
  ...
  sourcingItems, freightRequests, productOptions, productOptionItems,
} from "@/db/schema";
...
export async function recalcTotals(proposalId: number) {
...
export async function createProposal(formData: FormData) {
  ...
  const proposalNumber = await generateProposalNumber(series, customer?.legalName || "Customer");
...
export async function createRevision(proposalId: number) {
  ...
  const [revisionCustomer] = await db.select().from(customers).where(eq(customers.id, original.customerId)).limit(1);

  const proposalNumber = await generateProposalNumber(original.series, revisionCustomer?.legalName || "Customer");
...
export async function toggleProposalOption(proposalId: number, optionId: number, enable: boolean) {
  const session = await getSession();
  ...
  if (enable) {
    const items = await db
      .select({
        productId: productOptionItems.productId,
        quantity: productOptionItems.quantity,
        sortOrder: productOptionItems.sortOrder,
      })
      .from(productOptionItems)
      .where(eq(productOptionItems.optionId, optionId))
      .orderBy(asc(productOptionItems.sortOrder));

    if (items.length === 0) {
      throw new Error(`"${option.label}" has no bundle items yet — add some on the Proposal Options page first.`);
    }
    ...
    for (const item of items) {
      ...
      rows.push({
        ...
        sectionName: `${option.label} (Optional)`,
        description: product.customerFacingName || product.name,
        quantity: item.quantity,
        unitPrice: product.unitPrice,
        lineTotal: (qty * Number(product.unitPrice)).toFixed(2),
        sourceOptionId: option.id,
      });
    }
    if (rows.length > 0) {
      await db.insert(proposalLineItems).values(rows);
    }
    await logAudit({ userId: session?.userId, action: "enable_option", entityType: "proposal", entityId: proposalId, newValue: { optionId, label: option.label } });
  } else {
    await db
      .delete(proposalLineItems)
      .where(and(eq(proposalLineItems.proposalId, proposalId), eq(proposalLineItems.sourceOptionId, optionId)));
    await logAudit({ userId: session?.userId, action: "disable_option", entityType: "proposal", entityId: proposalId, newValue: { optionId, label: option.label } });
  }

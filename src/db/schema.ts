...
export const productSeriesEnum = pgEnum("product_series", [
  "adventure", "soar", "flex", "scape", "safe",
]);
...
export const customers = pgTable("customers", {
  ...
  mondaySourceId: varchar("monday_source_id", { length: 50 }).unique(),
  mondaySyncedAt: timestamp("monday_synced_at"),
  notes: text("notes"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
...
export const products = pgTable("products", {
  ...
  series: productSeriesEnum("series").notNull().default("adventure"),
...
export const proposals = pgTable("proposals", {
  ...
  series: productSeriesEnum("series"),
...
export const proposalLineItems = pgTable("proposal_line_items", {
  ...
  sourceOptionId: integer("source_option_id").references(() => productOptions.id),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});
...
// ---------- Product Options (toggleable add-ons per series, e.g. "Monkey Bars") ----------
// Turning an option on for a proposal auto-adds its bundle of line items; turning it off
// removes exactly those line items (tracked via proposalLineItems.sourceOptionId).
...
export const productOptionItems = pgTable("product_option_items", {
  id: serial("id").primaryKey(),
...
export const productOptionsRelations = relations(productOptions, ({ many }) => ({
  items: many(productOptionItems),
}));

export const productOptionItemsRelations = relations(productOptionItems, ({ one }) => ({
  option: one(productOptions, { fields: [productOptionItems.optionId], references: [productOptions.id] }),
  product: one(products, { fields: [productOptionItems.productId], references: [products.id] }),
}));

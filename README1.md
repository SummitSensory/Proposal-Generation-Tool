...
## Connecting Monday.com (customer sync)

Customers can be pulled in automatically from your "Deal Tracking" board on Monday.com instead
of typed in by hand — each Monday item becomes its own customer record (no merging of repeat
companies).

1. In Monday, click your avatar (bottom left) → **Admin** → **API** (or **Profile → Developers →
   My Access Tokens**) and generate a token. Read-only is enough.
2. In Vercel, add environment variable `MONDAY_API_TOKEN` with that token, and redeploy.
3. Visit `/api/setup?token=...` again (same token as before) if you haven't already — it also now
   seeds the standard vendor list and starter product catalog if they're missing.
4. On the **Customers** page, click **Sync from Monday.com** any time you want to pull the latest
   deals in. It also runs automatically every 30 minutes if you set a `CRON_SECRET` env var (any
   random string) — see `vercel.json`.

`MONDAY_BOARD_ID` defaults to the Deal Tracking board (6527740233); only set it if you want to
point at a different board.

## Adding products and their hardware (BOM) formulas

Each product belongs to one of five **series** (Summit Adventure Series, Soar, Flex, Scape, Safe)
— this determines which products show up when someone builds a proposal of that type. 248
starter products, pulled straight from your existing pricing workbook, are seeded automatically
via `/api/setup`. Products from Soar/Flex/Safe/Scape are seeded with **no vendor assigned** (the
source workbook didn't specify one for those) — the Products page flags these with a "Needs
vendor" badge; open each and assign Southpaw, TFH, Rope and Rescue, or whoever's correct.

Each product can also define:
...
## Proposal Options (auto-adding line items, e.g. "Monkey Bars")

Each series has a **Proposal Options** list (nav bar → Proposal Options) — toggleable add-ons.
Turning one on while building a proposal adds its whole bundle of parts as line items in one
click; turning it off removes exactly those lines. Quantities are fixed per option for now (they
don't scale with structure size yet) — edit an option's items any time to change what it adds.

Adventure Series options (Monkey Bars, Ladder Access, Zip Line, Climbing Wall & Safety
Accessories, Dual Trolley System, Column Wrap & Safety Padding, Quick Shift Bracket Hardware) are
pre-filled with real quantities pulled from an actual signed proposal (a 20'x10' structure) —
review them and adjust for your typical job size. Soar/Flex/Scape/Safe options were seeded as
empty shells (no reliable source data for those) — add their bundle items yourself on the
Proposal Options page.
